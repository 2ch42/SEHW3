...
class Account{
  ...
  public:
    ...
    static void addAccount(Account* newAccount);    // 회원가입 시 새로운 계정을 생성하는 static 함수
    static bool signIn(string userID, string password);     // 로그인 시, 계정 확인 및 로그인 진행하는 static 함수.
    static Account* getActiveAccount();     // 현재 로그인 중인 계정의 참조값을 반환 함수.
    static string deleteAccount();      // 회원 탈퇴 시, 로그인 중인 계정을 삭제하는 함수.
    static string signOut();        // 로그아웃 시, 현재 로그인 중인 계정을 NULL 로 설정 후 로그아웃 된 계정의 ID를 반환하는 함수.
};