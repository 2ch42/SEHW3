...
Account* Account::nowActiveAccount = NULL;      // 현재 로그인 중인 계정의 참조값을 갖는 static 포인터 변수의 초기화.
...
//로그인 시,  등록된 계정들의 참조값을 갖는 vector 배열에서 등록된 계정이 맞는 지 확인 후 true or false를 반환하는 함수. 
...
// 회원 가입 시, 계정 리스트에 새로 생성된 계정의 참조값 추가.
void Account::addAccount(Account* newAccount){
  accountList.push_back(newAccount);
}

// 회원 탈퇴 시, 현재 로그인 중인 계정 정보를 NULL로 설정 후, 계정을 delete하는 함수. 
 ...
 // 로그아웃 시 , 현재 로그인 중인 계정 정보를 NUL:L로 설정하는 함수.
...
//현재 로그인 중인 계정의 참조값을 반환하는 함수.
Account* Account::getActiveAccount(){
  return nowActiveAccount;
}

//현재 로그인 중인 계정의 이름 정보를 반환하는 함수. 
string Account::getActiveName(){
  return "Not logged in.";
}

//현재 로그인 중인 계정의 번호 정보를 반환하는 함수.
string Account::getActiveNum(){
  return "Not logged in.";
}

//현재 로그인 중인 계정의 ID 정보를 반환하는 함수.
